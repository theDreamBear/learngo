...
func TestServerPingPong(t *testing.T) {
	defer afterTest(t)
	s := NewServer(npc.HandlerFunc(func(w npc.ResponseWriter, r *npc.Request) {
	...
	for i := 0; i < 10; i++ {
		...
		for j := 0; j < 10; j++ {
			req := npc.NewRequest(strings.NewReader("ping"), 0)
			if _, err := req.Write(conn); err != nil {
				t.Fatalf("Write: %v", err)
			}
			resp, err := npc.ReadResponse(conn)
			if err != nil {
...
func TestClientWriteShutdown(t *testing.T) {
	defer afterTest(t)
	ts := NewServer(npc.HandlerFunc(func(w npc.ResponseWriter, r *npc.Request) {}))
	defer ts.Close()
	conn, err := net.Dial("tcp", ts.Listener.Addr().String())
	if err != nil {
		t.Fatalf("Dial: %v", err)
...
func xTestCloseNotifier(t *testing.T) {
	...
	ts := NewServer(npc.HandlerFunc(func(w npc.ResponseWriter, r *npc.Request) {
		gotReq <- true
		cc := w.(npc.CloseNotifier).CloseNotify()
		<-cc
		sawClose <- true
	}))
	defer ts.Close()
	...
	diec := make(chan bool)
	go func() {
		_, err := npc.NewRequest(strings.NewReader("ping"), 0).Write(conn)
		if err != nil {
			t.Fatalf("Write: %v", err)
		}
		<-diec
...
func TestAcceptMaxFds(t *testing.T) {
	...
	err := npc.Serve(ln, npc.HandlerFunc(func(npc.ResponseWriter, *npc.Request) {}))
	if err != io.EOF {
...
func xBenchmarkClientServer(b *testing.B) {
	...
	c := npc.NewClient([]string{ts.Listener.Addr().String()})
	defer c.Close()
	for i := 0; i < b.N; i++ {
		resp, err := c.Do(npc.NewRequest(strings.NewReader("ping"),0))
		if err != nil {
			b.Fatalf("Do: %v", err)
		}
